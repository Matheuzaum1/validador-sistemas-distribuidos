...
function Test-Classes {
    $serverClass = "target\classes\com\distribuidos\server\ServerMain.class"
    $clientClass = "target\classes\com\distribuidos\client\ClientMain.class"
...
function Get-ClassPath {
    ...
    # Adicionar JARs de dependencias se existirem (target/lib)
    if (Test-Path "target\lib") {
        $jars = Get-ChildItem "target\lib" -Filter "*.jar"
        foreach ($jar in $jars) {
...
function Start-ServerGUI {
    ...
    Start-Process java -ArgumentList "-cp", $classpath, "com.distribuidos.server.ServerMain" -WindowStyle Normal
...
function Start-ClientGUI {
    ...
    $classpath = Get-ClassPath
    Start-Process java -ArgumentList "-cp", $classpath, "com.distribuidos.client.ClientMain" -WindowStyle Normal
